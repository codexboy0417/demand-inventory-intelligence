...
def create_notebook():
    ...
    cells.append({
        ...
        "source": [
            ...
            "\n",
            "inv['stockout_gap'] = inv['forecast_lead_time_demand'] - inv['total_available_stock']\n",
            "inv['stockout_risk'] = np.clip(inv['stockout_gap'] / (inv['forecast_lead_time_demand'] + 1e-5), 0.0, 1.0)\n",
            "\n",
            "inv['overstock_gap'] = inv['on_hand_units'] - inv['forecast_6w_demand']\n",
            ...
            "inv['rupee_sales_at_risk'] = np.maximum(0, inv['stockout_gap']) * inv['list_price']\n",
            "inv['rupee_capital_locked'] = np.maximum(0, inv['overstock_gap']) * inv['unit_cost']\n",
            ...
            "print(f'Total Sales at Risk  : ₹{inv[\"rupee_sales_at_risk\"].sum():,.2f}')\n",
            "print(f'Total Capital Locked : ₹{inv[\"rupee_capital_locked\"].sum():,.2f}')\n",
            "\n",
